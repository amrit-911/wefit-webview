...
export default function UserSettingsPage() {
  ...
  const handleCmsPress = (item: CmsItem) => {
    ...
    if (link && link !== "NO-LINK" && (link.startsWith("http://") || link.startsWith("https://"))) {
      ...
      router.push(`/profile/cms-page/${item.id}`);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#121212] font-sans pt-10 px-5 pb-24 text-white">
      ...
      >
        <SettingsRow label="Edit Profile" onPress={() => router.push("/profile/edit-profile")} />
        <div className="h-px bg-white/5 mx-4" />
        <SettingsRow label="Change Password" onPress={() => router.push("/profile/change-password")} isLast />
      </motion.div>
          ...
          >
            {cmsLoading ? (
              ...
              <>
                ...
                <SettingsRow label="FAQs" onPress={() => router.push("/profile/faq")} isLast />
              </>
            )}
          </motion.div>
        </>
