...
import { ArrowLeft, ImagePlus, Trash2, X, ZoomIn, SlidersHorizontal, Loader2, Check } from "lucide-react";
...
function CompareSlider({ leftUrl, rightUrl, onClose }: { leftUrl: string; rightUrl: string; onClose: () => void }) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Right image — fills the full container, always visible */}
        ...
        {/* Left image — same size, clipped on the right by (100-pos)% */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img
          src={leftUrl}
          alt="Before"
...
export default function PhotoGalleryPage() {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      const uploaded = await Promise.all(files.map((f) => uploadGalleryPhoto(user.uid, f, selectedDate)));
      ...
      <div className="pt-12 px-5 pb-4 border-b border-white/5">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[12px] text-gray-500 font-medium">{photos.length} photo{photos.length !== 1 ? "s" : ""}</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="h-9 w-9 rounded-xl bg-[#a3e635] flex items-center justify-center shadow-[0_0_12px_rgba(163,230,53,0.3)]"
            >
              {uploading ? <Loader2 className="w-4 h-4 text-black animate-spin" /> : <ImagePlus className="w-4 h-4 text-black" />}
            </button>
          </div>
        </div>

        {/* Upload options — date picker */}
        <div className="flex items-center gap-3 mt-4">
          <div className="flex-1">
            <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5">Date for upload</p>
            <input
              type="date"
              value={selectedDate}
              onChange={(e) => setSelectedDate(e.target.value)}
              className="w-full bg-[#1c1c1e] border border-white/10 rounded-xl px-3 py-2 text-[12px] text-white focus:outline-none focus:border-[#a3e635]/50"
            />
          </div>
          <button
            onClick={() => fileInputRef.current?.click()}
            disabled={uploading}
            className="mt-5 h-10 px-4 bg-[#a3e635] disabled:opacity-50 hover:bg-[#b5f745] text-black font-bold text-[12px] rounded-xl flex items-center gap-2 transition-colors"
          >
            {uploading ? <><Loader2 className="w-3 h-3 animate-spin" />Uploading…</> : <><ImagePlus className="w-3 h-3" />Add Photos</>}
          </button>
        </div>
      ...
      <div className="px-5 pt-4">
        ...
        {!loading && photos.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-[12px] text-gray-500 mb-6">Upload your first progress photo</p>
            <button
              onClick={() => fileInputRef.current?.click()}
              className="h-11 px-6 bg-[#a3e635] text-black font-bold text-[13px] rounded-xl flex items-center gap-2"
            >
                  ...
                  >
                    ...
                    {!selectMode && (
                      <button
                        ...
                        className="absolute top-1.5 right-1.5 w-6 h-6 bg-black/60 rounded-full flex items-center justify-center hover:bg-red-500/80 transition-colors"
                      >
