...
export default function ClientChatPage() {
  ...
  // Step 1: Find client's trainer, then open/create room
  useEffect(() => {
    if (!user) return;
    ...
    getMemberById(user.uid)
      .then(async (member) => {
        const trainerId = member?.trainer;
        if (!trainerId) {
          setLoading(false);
          return;
        }
        // Check for existing room first (real-time)
        unsubRoom = subscribeToClientRoom(user.uid, async (existingRoom) => {
          if (existingRoom) {
            setRoom(existingRoom);
            setRoomId(existingRoom.id ?? null);
            setTrainerName(existingRoom.trainerName);
            setLoading(false);
          } else {
            // Room doesn't exist yet — create it
            // We need the trainer's name; use trainerId as fallback
            let tName = "Trainer";
            try {
              ...
              if (db) {
                ...
                if (snap.exists()) tName = (snap.data() as any).name || "Trainer";
              }
            } catch { /* ignore */ }
  ...
  // Step 2: Subscribe to messages once roomId is known
  useEffect(() => {
    if (!roomId) return;
  ...
  // Mark as read for client whenever messages load
  useEffect(() => {
    if (!roomId) return;
    markRoomRead(roomId, "client");
  ...
  const handleSend = useCallback(async () => {
    ...
    setSending(true);
    try {
      await sendMessage(roomId, user.uid, displayName || "Client", msg, "trainer");
    } catch (e) {
      console.error(e);
      setText(msg);
    } finally {
  ...
  const trainerInitial = trainerName.charAt(0).toUpperCase() || "T";
      ...
      <div className="flex items-center gap-3 px-5 pt-10 pb-4 border-b border-white/5 bg-[#121212] shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[14px] font-extrabold text-white leading-tight truncate">
            {loading ? "Connecting..." : trainerName || "Your Trainer"}
          </p>
          <p className="text-[10px] text-[#a3e635] font-medium">
            {loading ? "..." : "Active"}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-6">
        ...
        {!loading && !roomId && (
          <div className="flex flex-col items-center justify-center h-full gap-3 py-20">
            ...
            <p className="text-[13px] font-bold text-white">No trainer assigned</p>
            <p className="text-[11px] text-gray-500 text-center">
              Contact your trainer to get started with the chat.
            </p>
          </div>
        )}

        {!loading && roomId && messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full gap-3 py-20">
            <div className="w-16 h-16 rounded-full bg-[#1c1c1e] flex items-center justify-center">
              <span className="text-3xl">💬</span>
            </div>
            <p className="text-[13px] font-bold text-white">No messages yet</p>
            <p className="text-[11px] text-gray-500">Say hi to your trainer!</p>
          </div>
        )}
            ...
            <div className="space-y-2">
              {msgs.map((m, i) => {
                ...
                return (
                  ...
                  >
                    {!isMine && (
                      <div className="w-7 h-7 rounded-full bg-[#a3e635] flex items-center justify-center text-[11px] font-bold text-black shrink-0 mr-2 mt-auto">
                        {trainerInitial}
                      </div>
                    )}
                    <div className="max-w-[75%]">
                      <div
                        className={`px-4 py-2.5 rounded-2xl text-[13px] font-medium leading-relaxed ${
                          isMine
                      ...
                      <p className={`text-[9px] text-gray-600 mt-1 ${isMine ? "text-right" : "text-left"}`}>
