...
function MonthlyCheckinListContent() {
  const router = useRouter();
  const params = useParams();
  const searchParams = useSearchParams();
  const clientId = params?.id as string;
  const period = (searchParams.get("period") ?? "7days") as PeriodType;

  const [checkins, setCheckins] = useState<PeriodCheckinData[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!clientId) return;
    getPeriodCheckinsByPeriod(clientId, period)
      .then(setCheckins)
      .catch(console.error)
    ...
    <div className="min-h-screen bg-[#121212] font-sans pt-10 px-5 pb-12">
      ...
      {!loading && checkins.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[14px] font-bold text-white mb-1">No check-ins found</p>
          <p className="text-[12px] text-gray-500">
            No {PERIOD_LABELS[period].toLowerCase()} check-ins submitted yet
          </p>
        </div>
      )}
      ...
      {!loading && checkins.length > 0 && (
        ...
        >
          {checkins.map((checkin, idx) => {
            const dateParts = checkin.date.split("-");
            const monthIndex = parseInt(dateParts[1]) - 1;
            const monthName = MONTH_NAMES[monthIndex] ?? checkin.date;
            ...
            return (
              <button
                ...
                onClick={() =>
                  router.push(
                    `/trainer/clients/${clientId}/checkins/monthly/${checkin.id}`
                  )
                }
...
export default function TrainerMonthlyCheckinListPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#121212]" />}>
      <MonthlyCheckinListContent />
    </Suspense>
  );
}
