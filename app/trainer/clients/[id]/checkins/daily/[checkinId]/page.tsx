...
import { getCheckinById, saveCheckinFeedback, type CheckinData } from "@/lib/services/checkin.service";
...
export default function TrainerDailyCheckinDetailPage() {
  ...
  async function handleSendFeedback() {
    ...
    setSubmitting(true);
    try {
      await saveCheckinFeedback(checkin.id, feedbackText.trim());
      ...
      // Update local state so sent text is visible immediately
  ...
  return (
    <div className="min-h-screen bg-[#121212] font-sans pt-10 px-5 pb-36">
      ...
      <p className="text-[11px] text-gray-500 font-medium mb-6">Daily Check-In · Read Only</p>
      ...
      {!loading && !checkin && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-[12px] text-gray-500">This check-in may have been deleted</p>
        </div>
      )}
      ...
      {!loading && checkin && (
        ...
        >
          ...
          {/* Training */}
          <Field label="What Training Session Did You Do?" value={checkin.trainingSession} />
          ...
          {/* Note */}
          <Field label="Note" value={checkin.note} />
      ...
      {/* Done button */}
      {!loading && checkin && (
        <div className="fixed bottom-0 left-0 right-0 px-5 pb-8 bg-gradient-to-t from-[#121212] via-[#121212]/95 to-transparent pt-10 pointer-events-none">
          <button
